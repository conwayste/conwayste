...
pub struct Filter {
    ...
    auto_request_seqs:   HashSet<u64>, // Request sequences for auto-generated RequestActions (KeepAlives)
}
...
impl Filter {
                        ...
                        match notice {
                            ...
                            TransportNotice::EndpointIdle { endpoint } => {
                                ...
                                if self.mode.is_client() && !self.ping_endpoints.contains_key(&endpoint) {
                                    ...
                                    if let Err(e) = self.send_request_action_to_server(endpoint, action, RequestActionSource::Filter).await {
        ...
        match packet {
            Packet::Request {
                ...
                while let Some(request_action) = client.request_actions.take_if_matching(expected_seq_num.0) {
                    if !request_action.is_keep_alive() {
            ...
            Packet::Response {
                ...
                while let Some(response_code) = server.response_codes.take_if_matching(expected_seq_num.0) {
                    ...
                    if self.auto_request_seqs.take(&expected_seq_num.0).is_none() {
                        // The corresponding RequestAction was not generated by the Filter layer,
                        // so send the ResponseCode up to the app layer.
                        filter_notice_tx
                        ...
                        nwtrace!(
                            self,
                            "[F<-T,C] Not sending NewResponseCode for {:?} (sequence {}) since request was from this layer previously",
                            response_code,
                            expected_seq_num.0
                        );
                    }
    ...
    async fn process_filter_command(&mut self, command: FilterCmd) -> anyhow::Result<()> {
        match command {
            FilterCmd::SendRequestAction { endpoint, action } => {
                ...
                self.send_request_action_to_server(endpoint, action, RequestActionSource::App)
                    .await?
            }
    ...
    async fn send_request_action_to_server(
        ...
        if source.is_filter_layer() {
            // Save `sequence` so that Responses can not be sent up to App layer
            self.auto_request_seqs.insert(sequence);
        }
...
#[derive(Copy, Clone)]
enum RequestActionSource {
    /// Indicates normal flow
    App,
    // This is auto-generated (probably a KeepAlive)
    Filter,
}

impl RequestActionSource {
    fn is_filter_layer(self) -> bool {
        match self {
            RequestActionSource::Filter => true,
            _ => false,
        }
    }
}
