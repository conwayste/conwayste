...
fn seq_num_not_yet_known() {
    ...
    assert_eq!(
        determine_seq_num_advancement(1, &mut last_seen),
        SeqNumAdvancement::BrandNew
    );
}
...
fn seq_num_out_of_order() {
    ...
    assert_eq!(
        determine_seq_num_advancement(2, &mut last_seen),
...
fn seq_num_duplicate() {
    ...
    assert_eq!(
        determine_seq_num_advancement(0, &mut last_seen),
...
fn seq_num_wrapped_boundary_complex() {
    ...
    assert_eq!(
        determine_seq_num_advancement(u64::MAX - 2, &mut last_seen),
        SeqNumAdvancement::Duplicate
    );
    assert_eq!(
        determine_seq_num_advancement(u64::MAX, &mut last_seen),
        SeqNumAdvancement::Contiguous
    );
    assert_eq!(
        determine_seq_num_advancement(1, &mut last_seen),
